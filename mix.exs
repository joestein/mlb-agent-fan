defmodule MlbFan.MixProject do
  use Mix.Project
  ...
  def cli do
    [
      preferred_envs: [precommit: :test, "test.unit": :test]
    ]
  end
  ...
  defp deps do
    [
      ...
      {:dns_cluster, "~> 0.2.0"},
      {:bandit, "~> 1.5"},
      # HTTP client for Anthropic / Exa / MLB Stats (streaming SSE)
      {:req, "~> 0.5"},
      # Agent framework (mandated)
      {:jido, "~> 2.3"},
      # MCP server + client, Streamable HTTP (mandated)
      {:hermes_mcp, "~> 0.14"},
      # Markdown -> HTML for chat, with output sanitization
      {:earmark, "~> 1.4"},
  ...
  defp aliases do
    [
      ...
      # DB-free unit suite for fast feedback (streaks, pricing, sse, parsers,
      # cache keys, catalog, exa) — runs without Postgres.
